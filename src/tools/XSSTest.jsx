...
export default function XSSTest({ onBack }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const { data: supaData, error } = await supabase.from('xss_logs').select('*').order('created_at', { ascending: false }).limit(20);
    ...
    setData({ status: 'DEMO_MODE', message: 'Tambahkan VITE_CLOUDFLARE_WORKER_URL di .env.local', timestamp: new Date().toISOString(), endpoint: '/api/xss' });
    setSource('Demo Mode');
    setLoading(false);
  };
    ...
    <div style={{minHeight: '100vh', background: '#050505', color: '#00ff9d', padding: '20px', fontFamily: 'Courier New, monospace'}}>
      <div style={{maxWidth: '1200px', margin: '0 auto'}}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', paddingBottom: '1rem', borderBottom: '1px solid #00ff9d33', flexWrap: 'wrap', gap: '1rem'}}>
          <div>
            <h1 style={{margin: 0, fontSize: '1.5rem'}}>💉 XSSTest</h1>
            <p style={{margin: '0.3rem 0 0', color: '#00ff9d88', fontSize: '0.85rem'}}>XSS Vulnerability Scanner</p>
