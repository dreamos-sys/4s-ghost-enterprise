export default function BotDetect() {
  return (
    <div style={{background:'#020617',minHeight:'100vh',color:'#e2e8f0',padding:'2rem',fontFamily:'Inter,sans-serif'}}>
      <h1 style={{color:'#00ff9d',fontFamily:'Orbitron,monospace'}}>🤖 Bot Detector</h1>
      <p>Bots blocked today: <strong style={{color:'#ef4444'}}>1,892</strong></p>
      <p>Legitimate users: <strong style={{color:'#10b981'}}>456</strong></p>
      <p>Detection rate: <strong>99.2%</strong></p>
      <a href="/" style={{display:'block',marginTop:'1rem',color:'#0ea5e9'}}>← Dashboard</a>
    </div>
  );
}
